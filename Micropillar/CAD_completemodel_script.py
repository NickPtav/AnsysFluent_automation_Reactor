...
min_point =  Point.Create(MM(0), MM(alt_reac + 1.5), MM(-0.1))
max_point = Point.Create(MM(compri_reac), MM(alt_reac + 2), MM(larg_reac))

BoundingBox = Box.Create(min_point, max_point)
faces = comp.ConvertToFaces().FilterByBoundingBox(BoundingBox)
faces.CreateAGroup('cut')

# Extrudar 1 face
selection = FaceSelection.CreateByGroups('cut')
options = ExtrudeFaceOptions()
options.ExtrudeType = ExtrudeType.ForceCut
result = ExtrudeFaces.Execute(selection, MM(-alt_reac - 1), options, Info1)
# EndBlock
...
# Definir plano de esboço
sectionPlane = Plane.Create(Frame.Create(Point.Create(MM(1), MM(1), MM(1)), 
    Direction.DirZ, 
    Direction.DirX))
result = ViewHelper.SetSketchPlane(sectionPlane, None)
...
# Criar nomenclatura de entradas e saídas
# Creating inlet and outlet group to extrusion
# Definir plano de esboço
...
min_point =  Point.Create(MM(-0.3), MM(-0.1), MM(-0.1))
max_point = Point.Create(MM(-0.1), MM(alt_reac + 2.1), MM(larg_reac + 2.1))
...
result = ExtrudeFaces.Execute(selection, MM(-0.2), options, Info1)
# EndBlock

# 1 - Creating Outlet

sectionPlane = Plane.Create(Frame.Create(Point.Create(MM(compri_reac + 2.2), MM(0), MM(0)), 
    Direction.DirZ, 
    Direction.DirY))
result = ViewHelper.SetSketchPlane(sectionPlane, None)
...
min_point =  Point.Create(MM(compri_reac + 2.1), MM(-0.1), MM(-0.1))
max_point = Point.Create(MM(compri_reac + 2.2), MM(alt_reac + 0.1), MM(larg_reac + 2))
...
# Vidro
# Definir plano de esboço
sectionPlane = Plane.Create(Frame.Create(Point.Create(MM(0), MM(alt_reac + 2), MM(0)), 
    Direction.DirZ, 
    Direction.DirX))
result = ViewHelper.SetSketchPlane(sectionPlane, None)
...
min_point =  Point.Create(MM(0), MM(alt_reac + 2.1), MM(-0.1))
max_point = Point.Create(MM(compri_reac), MM(alt_reac + 1.9), MM(larg_reac))

BoundingBox = Box.Create(min_point, max_point)
faces = comp.ConvertToFaces().FilterByBoundingBox(BoundingBox)
faces.CreateAGroup('vidro')

# Extrudar 1 face
selection = FaceSelection.CreateByGroups('vidro')
options = ExtrudeFaceOptions()
options.ExtrudeType = ExtrudeType.ForceIndependent
...
#----------------
int_area = larg_reac * alt_reac
tampa_area = (larg_reac + 2) * (compri_reac + 2)


comp = Selection.Create(GetRootPart())
faces = comp.ConvertToFaces().FilterByArea(MM2(int_area - 0.001),MM2(int_area + 0.001))
groupCanais = faces.CreateAGroup('int')

comp = Selection.Create(GetRootPart())
faces = comp.ConvertToFaces().FilterByArea(MM2(tampa_area - 0.001),MM2(tampa_area + 0.001))
groupCanais = faces.CreateAGroup('tampa')

# Criar volume
options = VolumeExtractOptions()
selection = FaceSelection.CreateByGroups('tampa')
secondarySelection = FaceSelection.CreateByGroups('int', 'int')
result = VolumeExtract.Create(selection, secondarySelection, options)
#EndBlock



# Criar grupo de seleção nomeado
primarySelection = Face5
secondarySelection = Selection.Empty()
result = NamedSelection.Create(primarySelection, secondarySelection, "Grupo1")
# EndBlock

result = NamedSelection.Rename("Grupo1", "inlet")
# EndBlock

# Criar grupo de seleção nomeado
primarySelection = Face6
secondarySelection = Selection.Empty()
result = NamedSelection.Create(primarySelection, secondarySelection, "Grupo2")
# EndBlock
...
result = NamedSelection.Rename("Grupo2", "outlet")
# EndBlock
